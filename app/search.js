//[Last Update: 11:13 AM 5/28/2026]
...
async function executeSearch(runSets, baseFields, dateFilter, labelPrefix, sessionToken, excludeGroup) {
          ...
          const SERVER_CAP = 10000;
          const MAX_DEPTH = 10;
          const searchStartDate = dateFilter.value.firstValue.slice(0, 10);
          const searchEndDate = dateFilter.value.secondValue.slice(0, 10);
          async function fetchRange(startDate, endDate, baseFilters, setRows, depth, si) {
            const rangeFilter = { parameterName: "recordedDateTime", operator: "BETWEEN", type: "DATE", value: { firstValue: isoStart(startDate), secondValue: isoEnd(endDate) } };
            const interactionFilters = [...baseFilters, rangeFilter];
            let from = 0;
            const rangeRows = [];
            while (true) {
              ...
              for (let ri = 0; ri < rows.length; ri++) rangeRows.push(rows[ri]);
              totalFetched += rows.length;
              progressUI.set(
                "Searching (" + labelPrefix + " " + (si + 1) + "/" + totalRuns + ")...",
                Math.min(80, 25 + Math.floor((si / Math.max(1, totalRuns)) * 55)),
                "Set: " + (setRows.length + rangeRows.length) + " \u2022 Total: " + totalFetched
              );
              if (rangeRows.length >= MAX_ROWS || rows.length < PAGE_SIZE) break;
              from += PAGE_SIZE;
              await sleep(250);
            }
            if (rangeRows.length >= SERVER_CAP) {
              const startMs = new Date(startDate + "T00:00:00Z").getTime();
              const endMs = new Date(endDate + "T00:00:00Z").getTime();
              if (depth < MAX_DEPTH && endMs - startMs >= 86400000) {
                totalFetched -= rangeRows.length;
                const midMs = Math.floor((startMs + endMs) / 2);
                const midDate = new Date(midMs).toISOString().slice(0, 10);
                const dayAfterMid = new Date(midMs + 86400000).toISOString().slice(0, 10);
                progressUI.set(
                  "Subdividing " + startDate + " \u2192 " + endDate + " (depth " + (depth + 1) + ")...",
                  Math.min(80, 25 + Math.floor((si / Math.max(1, totalRuns)) * 55)),
                  "Hit " + SERVER_CAP + " cap, splitting range"
                );
                const r1 = await fetchRange(startDate, midDate, baseFilters, setRows, depth + 1, si);
                if (r1 === null) return null;
                const r2 = await fetchRange(dayAfterMid, endDate, baseFilters, setRows, depth + 1, si);
                if (r2 === null) return null;
                return true;
              }
              console.warn("[Search] Range " + startDate + " to " + endDate + " returned " + rangeRows.length + " rows (server cap). Cannot subdivide further.");
            }
            for (let ri = 0; ri < rangeRows.length; ri++) setRows.push(rangeRows[ri]);
            return true;
          }
          for (let si = 0; si < runSets.length; si++) {
            ...
            for (let ei = 0; ei < phraseExpansions.length; ei++) {
              ...
              const baseFilters = [];
              if (runSet.keywordGroup) baseFilters.push(runSet.keywordGroup);
              if (expansion.group) baseFilters.push(expansion.group);
              if (excludeGroup) baseFilters.push(excludeGroup);
              const setRows = [];
              const result = await fetchRange(searchStartDate, searchEndDate, baseFilters, setRows, 0, si);
