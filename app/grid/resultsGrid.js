  ...
  function openResultsGrid() {
    (async () => {
      try {
        ...
        const LS_LAYOUTS_KEY = "nexidia_grid_layouts";
        function getSavedLayouts() {
          try { const o = JSON.parse(localStorage.getItem(LS_LAYOUTS_KEY) || "{}"); return (o && typeof o === "object") ? o : {}; } catch (_) { return {}; }
        }
        function writeSavedLayouts(obj) {
          try { localStorage.setItem(LS_LAYOUTS_KEY, JSON.stringify(obj)); } catch (_) {}
        }
        function setClickMode(mode) {
          state.clickMode = mode === "select" ? "select" : "play";
        ...
        function captureLayout(name) {
          return {
            ...
            sorts: state.sorts.map((s) => ({ field: s.field, dir: s.dir === -1 ? -1 : 1 })),
            columnFilters: JSON.parse(JSON.stringify(state.columnFilters || {})),
            colWidths: [...state.colWidths.entries()],
            globalFilter: state.globalFilter || "",
            clickMode: state.clickMode
          };
        }
        function applyLayout(layout) {
          if (!layout || !Array.isArray(layout.fields)) { alert("This layout could not be read."); return; }
          const currentHeaders = new Map();
          state.fields.forEach((f, i) => currentHeaders.set(f, state.headers[i]));
          const present = new Set(state.fields);
          const ordered = [], orderedHeaders = [], used = new Set();
          for (const f of layout.fields) {
            if (present.has(f) && !used.has(f)) { ordered.push(f); orderedHeaders.push(currentHeaders.get(f)); used.add(f); }
          }
          for (let i = 0; i < state.fields.length; i++) {
            const f = state.fields[i];
            if (!used.has(f)) { ordered.push(f); orderedHeaders.push(state.headers[i]); used.add(f); }
          }
          state.fields = ordered; state.headers = orderedHeaders;
          const inLayout = new Set(layout.fields);
          const visSet = new Set(Array.isArray(layout.visible) ? layout.visible : []);
          state.visible = new Set();
          for (const f of state.fields) {
            if (inLayout.has(f)) { if (visSet.has(f)) state.visible.add(f); }
            else state.visible.add(f);
          }
          state.colWidths = new Map();
          if (Array.isArray(layout.colWidths)) {
            for (const pair of layout.colWidths) { if (Array.isArray(pair) && present.has(pair[0])) state.colWidths.set(pair[0], pair[1]); }
          }
          state.sorts = Array.isArray(layout.sorts) ? layout.sorts.filter((s) => s && present.has(s.field)).map((s) => ({ field: s.field, dir: s.dir === -1 ? -1 : 1 })).slice(0, 3) : [];
          state.columnFilters = {};
          if (layout.columnFilters && typeof layout.columnFilters === "object") {
            for (const f of Object.keys(layout.columnFilters)) { if (present.has(f)) state.columnFilters[f] = layout.columnFilters[f]; }
          }
          state.globalFilter = typeof layout.globalFilter === "string" ? layout.globalFilter : "";
          globalSearchBox.value = state.globalFilter;
          if (layout.clickMode === "select" || layout.clickMode === "play") setClickMode(layout.clickMode);
          rebuildColumnPanel(); renderSortBadges(); recomputeAndRender();
        }
        function downloadLayoutText(filename, text) {
          try {
          ...
          } catch (_) { alert("Could not create the export file."); }
        }
        function safeLayoutFileName(name) { return (name || "layout").replace(/[^a-z0-9_\-]+/gi, "_").slice(0, 60); }
        function exportOneLayout(name) {
          const all = getSavedLayouts();
          if (!all[name]) { alert("Layout not found."); return; }
          downloadLayoutText("nexidia_grid_layout_" + safeLayoutFileName(name) + ".txt", JSON.stringify(all[name], null, 2));
        }
        function exportAllLayouts() {
          const all = getSavedLayouts();
          if (!Object.keys(all).length) { alert("No saved layouts to export."); return; }
          downloadLayoutText("nexidia_grid_layouts.txt", JSON.stringify({ type: "nexidiaGridLayoutBundle", version: 1, layouts: all }, null, 2));
        }
        function importLayoutsFromText(text) {
          let parsed;
          try { parsed = JSON.parse(text); } catch (_) { alert("That file is not a valid layout file."); return 0; }
          const all = getSavedLayouts();
          let added = 0;
          if (parsed && parsed.type === "nexidiaGridLayoutBundle" && parsed.layouts && typeof parsed.layouts === "object") {
            for (const k of Object.keys(parsed.layouts)) {
              const lay = parsed.layouts[k];
              if (lay && Array.isArray(lay.fields)) { let nm = k; while (all[nm] && all[nm].savedAt !== lay.savedAt) nm = nm + " (imported)"; all[nm] = lay; added++; }
            }
          } else if (parsed && Array.isArray(parsed.fields)) {
            let nm = parsed.name || "Imported Layout";
            while (all[nm]) nm = nm + " (imported)";
            all[nm] = parsed; added++;
          } else { alert("Unrecognized layout file format."); return 0; }
          writeSavedLayouts(all);
          return added;
        }
        function openLayoutManager() {
          document.querySelectorAll("[data-layout-mgr]").forEach((m) => m.remove());
          const overlay = el("div", { style: "position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:1000004;display:flex;align-items:center;justify-content:center;font-family:Segoe UI,Arial,sans-serif;" });
          overlay.setAttribute("data-layout-mgr", "1");
          const box = el("div", { style: "background:#fff;width:460px;max-width:94vw;max-height:82vh;overflow:auto;border-radius:12px;padding:22px;box-shadow:0 8px 24px rgba(0,0,0,.3);" });
          box.appendChild(el("div", { style: "font-size:15px;font-weight:700;color:#111827;margin-bottom:4px;" }, "Grid Layouts"));
          box.appendChild(el("div", { style: "font-size:11px;color:#6b7280;line-height:1.4;margin-bottom:14px;" }, "Save column order, widths, visibility, sorts, and filters. Layouts are stored in this browser and can be exported to a file for backup or sharing."));
          const saveRow = el("div", { style: "display:flex;gap:6px;margin-bottom:14px;" });
          const nameInput = el("input", { type: "text", placeholder: "Layout name...", style: "flex:1;padding:8px 10px;border:1px solid #ccc;border-radius:6px;box-sizing:border-box;font-size:13px;" });
          const saveBtn = el("button", { style: "padding:8px 14px;border-radius:8px;border:0;background:linear-gradient(135deg,#1d4ed8,#3b82f6);color:#fff;font-size:13px;font-weight:600;cursor:pointer;flex-shrink:0;" }, "Save");
          const listWrap = el("div", { style: "border:1px solid #e5e7eb;border-radius:8px;margin-bottom:14px;max-height:280px;overflow-y:auto;" });
          function renderLayoutList() {
            listWrap.innerHTML = "";
            const all = getSavedLayouts();
            const names = Object.keys(all).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
            if (!names.length) { listWrap.appendChild(el("div", { style: "padding:12px;font-size:12px;color:#6b7280;" }, "No saved layouts yet.")); return; }
            for (const nm of names) {
              const lay = all[nm];
              const visCount = Array.isArray(lay.visible) ? lay.visible.length : 0;
              const sortCount = Array.isArray(lay.sorts) ? lay.sorts.length : 0;
              const filterCount = lay.columnFilters ? Object.keys(lay.columnFilters).length : 0;
              const savedTxt = lay.savedAt ? new Date(lay.savedAt).toLocaleDateString() : "";
              const row = el("div", { style: "display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid #f1f5f9;" });
              const info = el("div", { style: "flex:1;min-width:0;" });
              info.appendChild(el("div", { style: "font-size:13px;font-weight:600;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" }, nm));
              info.appendChild(el("div", { style: "font-size:10px;color:#6b7280;" }, visCount + " cols \u00B7 " + sortCount + " sorts \u00B7 " + filterCount + " filters" + (savedTxt ? " \u00B7 " + savedTxt : "")));
              const loadB = el("button", { style: "padding:5px 10px;border-radius:6px;border:1px solid #3b82f6;background:#fff;color:#1d4ed8;font-size:11px;font-weight:600;cursor:pointer;flex-shrink:0;" }, "Load");
              const expB = el("button", { style: "padding:5px 8px;border-radius:6px;border:1px solid #d1d5db;background:#f9fafb;color:#374151;font-size:11px;cursor:pointer;flex-shrink:0;" }, "Export");
              const delB = el("button", { style: "padding:5px 8px;border-radius:6px;border:1px solid #fca5a5;background:#fff;color:#b91c1c;font-size:11px;cursor:pointer;flex-shrink:0;" }, "Delete");
              loadB.onclick = () => { applyLayout(all[nm]); overlay.remove(); };
              expB.onclick = () => exportOneLayout(nm);
              delB.onclick = () => { const a2 = getSavedLayouts(); delete a2[nm]; writeSavedLayouts(a2); renderLayoutList(); };
              row.appendChild(info); row.appendChild(loadB); row.appendChild(expB); row.appendChild(delB);
          ...
          saveBtn.onclick = () => {
            ...
            const all = getSavedLayouts();
            if (all[nm] && !confirm("A layout named \"" + nm + "\" already exists. Overwrite it?")) return;
            all[nm] = captureLayout(nm); writeSavedLayouts(all); nameInput.value = ""; renderLayoutList();
          };
          nameInput.addEventListener("keydown", (e) => { if (e.key === "Enter") saveBtn.onclick(); });
          saveRow.appendChild(nameInput); saveRow.appendChild(saveBtn);
          const fileRow = el("div", { style: "display:flex;gap:6px;" });
          const importBtn = el("button", { style: "flex:1;padding:8px;border-radius:8px;border:1px solid #6366f1;background:#fff;color:#4f46e5;font-size:12px;font-weight:600;cursor:pointer;" }, "Import from File");
          const exportAllBtn = el("button", { style: "flex:1;padding:8px;border-radius:8px;border:1px solid #22c55e;background:#fff;color:#16a34a;font-size:12px;font-weight:600;cursor:pointer;" }, "Export All");
          const fileInput = el("input", { type: "file", accept: ".txt,.json,application/json,text/plain", style: "display:none;" });
          importBtn.onclick = () => fileInput.click();
          fileInput.onchange = () => {
            const file = fileInput.files && fileInput.files[0];
            if (!file) return;
            const reader = new FileReader();
            reader.onload = () => { const n = importLayoutsFromText(String(reader.result || "")); if (n > 0) { renderLayoutList(); alert(n + " layout" + (n === 1 ? "" : "s") + " imported."); } };
            reader.onerror = () => alert("Could not read that file.");
            reader.readAsText(file);
            fileInput.value = "";
          };
          exportAllBtn.onclick = () => exportAllLayouts();
          fileRow.appendChild(importBtn); fileRow.appendChild(exportAllBtn);
          const closeBtn = el("button", { style: "width:100%;margin-top:12px;padding:8px;border-radius:8px;border:1px solid #e5e7eb;background:#f9fafb;cursor:pointer;font-size:13px;color:#6b7280;" }, "Close");
          closeBtn.onclick = () => overlay.remove();
          box.appendChild(saveRow); box.appendChild(listWrap); box.appendChild(fileRow); box.appendChild(fileInput); box.appendChild(closeBtn);
          renderLayoutList();
          overlay.appendChild(box); document.body.appendChild(overlay);
        ...
        const layoutBtn = el("button", { style: "padding:6px 10px;border-radius:8px;border:1px solid #0ea5e9;background:#fff;color:#0369a1;cursor:pointer;font-size:12px;flex-shrink:0;" }, "\u2699 Layouts");
        layoutBtn.onclick = () => openLayoutManager();
