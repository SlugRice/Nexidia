...
(() => {
  ...
  //##> The launcher is now a reusable function instead of a one-shot IIFE body, so tools
  //##> opened programmatically (e.g., Batch Builder's "Back to Main Menu") can bring it
  //##> back. It is exposed via api.setShared("openEntryMenu", openLauncher); batchBuilder
  //##> resolves that shared function first.
  function openLauncher() {
    //##> Reopen guard. If the flag is stale (modal already gone) clear it so a later call
    //##> can reopen. Only bail if a launcher modal is actually on screen.
    if (window.__NEXIDIA_LAUNCHER_OPEN__ && document.querySelector("[data-nexidia-launcher]")) return;
    ...
    `});
    modal.dataset.nexidiaLauncher = "1";
  ...
  //##> Expose the reopen hook for programmatic returns (Batch Builder "Back to Main Menu").
  api.setShared("openEntryMenu", openLauncher);
  //##> Auto-open once at bootstrap (original behavior).
  openLauncher();
})();
